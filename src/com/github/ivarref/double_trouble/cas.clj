...
(defn sha-exists? [db sha]
  (some? (d/q '[:find ?tx .
                :in $ ?sha
                :where
                [?tx :com.github.ivarref.no-more-double-trouble/sha-1 ?sha]]
              db
              sha)))
      ...
      (if (= curr-val old-val)
        (if (sha-exists? db sha)
          (d/cancel {:cognitect.anomalies/category                         :cognitect.anomalies/incorrect
                     ...
                     :com.github.ivarref.no-more-double-trouble/sha        sha})
          [[:db/add lookup-ref a new-val]
           {:db/id "datomic.tx" :com.github.ivarref.no-more-double-trouble/sha-1 sha}])
        ...
        (let [new-val-write (d/q '[:find ?new-val .
          ...
          (if (some? new-val-write)
            (d/cancel {:cognitect.anomalies/category                            :cognitect.anomalies/conflict
                       ...
                       :com.github.ivarref.no-more-double-trouble/e             e
                       :com.github.ivarref.no-more-double-trouble/a             a
                       :com.github.ivarref.no-more-double-trouble/old-val       old-val
                       :com.github.ivarref.no-more-double-trouble/new-val       new-val
                       :com.github.ivarref.no-more-double-trouble/new-val-write new-val-write
                       :com.github.ivarref.no-more-double-trouble/sha           sha})
            #_()))))
